  ...
  describe('Auth', () => {
    ...
    describe('Signin', () => {
      ...
      it('should signin with correct data', async () => {
        ...
        const response2 = await request(app.getHttpServer()).post('/auth/signin').send({
          cnic: '3740500000001',
          password: 'Password@123',
        });
      ...
      it('should get me', async () => {
        const response = await request(app.getHttpServer()).get('/users/accounts').auth(token1, { type: 'bearer' });
  ...
  describe('Users', () => {
    describe('Accounts', () => {
      it('should get all user accounts', async () => {
        ...
        expect(response.body.data.accounts[0].accountNumber).toBe(accountNumber1);

        const response2 = await request(app.getHttpServer()).get('/users/accounts').auth(token2, { type: 'bearer' });
        ...
        expect(response2.body.data.accounts[0].accountNumber).toBe(accountNumber2);
      });
    });
  });
});
